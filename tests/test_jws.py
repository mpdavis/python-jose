...
"""
Note: Unlike the ecdsa library, pyelliptic is much stricter about key formats
and values. The following keys were generated from openssl and should be more
valid than ones generated from ecdsa.

These keys can be generated by the following:
    ```
    # Make a new private key based off of prime256v1
    # (You can get a list of valid curves via
    # `openssl ecparam -list_curves` )
    for name in bob alice
    do
        openssl ecparam -name prime256v1 -genkey -noout -out ec_key_$name.pem
        # And generate it's matching public key.
        openssl ec -in ec_key_$name.pem -pubout -out ec_pub_$name.pem

        # verify that the
        openssl ecparam -name prime256v1 -check -in ec_pub_$name.pem
        openssl ecparam -name prime256v1 -check -in ec_key_$name.pem
        echo $name generated...
    done
    ```

    I am using alice's private key and bob's public key.
...
/4iczJnzJhUKDqHe5dJ//PLd7R3WVHxkvv7jFNTK
Yg==
-----END EC PRIVATE KEY-----""",
"secp384r1": """-----BEGIN EC PRIVATE KEY-----
MIGkAgEBBDCoukBVqLgAKA2IigLGcWo27vN85DxAOKIwM5gBoxWuyuI6ZQS6YIKS
yrqtliw6yR+gBwYFK4EEACKhZANiAATbuZnWkRlcYwjJqZsDZRpleks/moCvnImM
lAubhcO5mYnwnRCNpZTJ2guYqlo8yAWPuS9EOCK0SoiRlNwu8XHEM+B+z0elhfTv
jpYNfweolpWtTBQf7XE7p33d64iftLE=
-----END EC PRIVATE KEY-----""",
"prime256v1": """-----BEGIN EC PRIVATE KEY-----
MHcCAQEEIFJIzWiO+vJFrlufiGGQyDaurrxXfFM6zsyKc66T1IY6oAoGCCqGSM49
AwEHoUQDQgAE0gm4od23MedumS+rXQwVa33NCGt0+l2+GJ+yHQhj9f2R7f7b828w
ZiSuiUB9JfN9CPa5WorvI2KCTYqjRlTRDA==
-----END EC PRIVATE KEY-----
"""
}
ec_public_keys = {
"secp521r1": """-----BEGIN PUBLIC KEY-----
...
3e0d1lR8ZL7+4xTUymI=
-----END PUBLIC KEY-----""",
"secp384r1": """-----BEGIN PUBLIC KEY-----
MHYwEAYHKoZIzj0CAQYFK4EEACIDYgAE27mZ1pEZXGMIyambA2UaZXpLP5qAr5yJ
jJQLm4XDuZmJ8J0QjaWUydoLmKpaPMgFj7kvRDgitEqIkZTcLvFxxDPgfs9HpYX0
746WDX8HqJaVrUwUH+1xO6d93euIn7Sx
-----END PUBLIC KEY-----""",
"prime256v1": """-----BEGIN PUBLIC KEY-----
MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAE0gm4od23MedumS+rXQwVa33NCGt0
+l2+GJ+yHQhj9f2R7f7b828wZiSuiUB9JfN9CPa5WorvI2KCTYqjRlTRDA==
-----END PUBLIC KEY-----"""
}
...
class TestEC(object):
    ...
    def test_EC256(self, payload):
        token = jws.sign(
            payload,
            ec_private_keys["prime256v1"],
            algorithm=ALGORITHMS.ES256)
        assert jws.verify(
            token,
            ec_public_keys["prime256v1"],
            ALGORITHMS.ES256) == payload

    def test_EC384(self, payload):
        token = jws.sign(
            payload,
            ec_private_keys["secp384r1"],
            algorithm=ALGORITHMS.ES384)
        assert jws.verify(
            token,
            ec_public_keys["secp384r1"],
            ALGORITHMS.ES384) == payload

    def test_EC512(self, payload):
        token = jws.sign(
            payload,
            ec_private_keys["secp521r1"],
            algorithm=ALGORITHMS.ES512)
        assert jws.verify(
            token,
            ec_public_keys["secp521r1"],
            ALGORITHMS.ES512) == payload

    def test_wrong_alg(self, payload):
        token = jws.sign(payload, ec_private_keys["secp521r1"],
                         algorithm=ALGORITHMS.ES256)
        with pytest.raises(JWSError):
